module Calm
  module Routing
    class Map
      ...
      def path(path : String, target : Calm::Handler::ApplicationHandler.class | Map, action : Symbol | Symbol, name : String | Symbol) : Nil
        ...
        if target.is_a?(Calm::Handler::ApplicationHandler.class)
          rule = Rule::Path.new(path, target, action, name.to_s)
        else # Nested routes map
          rule = Rule::Map.new(path, target, action, name.to_s)
        end
